...
- **Local branch tip:** `f8603ea` (plan for 2026-06-04 sync, just committed)
...
- `git reset --hard f8603ea` — drops all 6 picks, returns to pre-sync state
...
| `git diff --stat f8603ea..HEAD`         | Upstream changes + SYNC-NOTES, no sandbox churn                        |
| `git diff f8603ea..HEAD -- .claude/`    | Tracked `.claude/` content matches `main:.claude/` (sandbox unchanged) |
